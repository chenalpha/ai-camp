...
def chat_with_memory(message: str, session_id: str) -> dict:
    ...
    payload = {
        "model": "qwen3:8b",  # 选择最小的qwen3模型，减少计算时间
        "messages": context_messages,  # 传入包含历史上下文的消息列表
        "stream": False,  # 非流式响应，直接获取完整回复
        "temperature": 0.0,  # 温度设为0，避免随机回复，保证测试稳定
        "max_tokens": 64  # 限制回复长度，仅保留核心信息
    }
    ...
    with httpx.Client(timeout=30.0) as client:
        response = client.post(
            ...
            content=json.dumps(payload)  # 序列化请求体为JSON格式
        )
        ai_response = response.json()["message"]["content"].strip()  # 提取AI回复内容
